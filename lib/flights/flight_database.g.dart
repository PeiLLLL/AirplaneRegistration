    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Flight` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `flightName` TEXT NOT NULL, `departureCity` TEXT NOT NULL, `destinationCity` TEXT NOT NULL, `departureTime` TEXT NOT NULL, `arrivalTime` TEXT NOT NULL, `createdAt` TEXT NOT NULL)');
...
class _$FlightDao extends FlightDao {
  ...
  Stream<Flight?> findByDeparture(String departure) {
    return _queryAdapter.queryStream(
        'SELECT * FROM Flight WHERE departure_city = ?1',
        mapper: (Map<String, Object?> row) => Flight(
            id: row['id'] as int?,
            flightName: row['flightName'] as String,
            departureCity: row['departureCity'] as String,
  ...
  Stream<Flight?> findByDestination(String destination) {
    return _queryAdapter.queryStream(
        'SELECT * FROM Flight WHERE destination_city = ?1',
        mapper: (Map<String, Object?> row) => Flight(
            id: row['id'] as int?,
            flightName: row['flightName'] as String,
            departureCity: row['departureCity'] as String,
